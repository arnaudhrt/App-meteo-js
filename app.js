...
function callApiMap() {
    fetch(`http://api.openweathermap.org/geo/1.0/direct?q=${valueInputCity}&limit=5&appid=${keyApiWeather}`)
        .then((answer) => {
            return answer.json()
        })
        .then((data) => {
            ...
            long = resultApiMap.lon
            lat = resultApiMap.lat
            city.innerText = resultApiMap.name
            country.innerText = `${resultApiMap.country}`
